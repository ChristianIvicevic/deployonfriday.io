...
> Next.js extends with React with server-side rendering and static site generation capabilities as well as offering features such as internationalization, analytics etc. out-of-the-box.
...
Server-side rendering however is crucial when you want to perform search engine optimization, one of the reasons it is often preferred for certain applications that have dynamic content that has to be indexed often.
...
Unfortunately I haven't invested enough time to give a credible and thorough comparison, nevertheless I wanted to share which issues I had with Gatsby.*
...
>
> In stark contrast to that, Next.js provides you with a clean boilerplate that doesn't force you to follow conventions for styling, components, pages or data.
...
I got it running pretty quickly which is always pleasant when trying out new technology for the first time.
...
The next wish I had was to adjust the templates slightly - basically I wanted to replace some elements in the footer.
Eventually I figured out, that in order to replace the footer originally located in `node_modules/theme-package/dir/footer.mdx` I had to create a file at `theme-package/dir/footer.mdx` with custom content.
...
Alas, this seemed like the most common way people start out with Gatsby.**
...
No particular styling approach has really impressed me thus I always stick to classic SCSS files using the BEM naming convention.
...
Features like server-side rendering or static site generation are opt-in and enabled by the existance of special methods in corresponding files.
...
The only significant difference was that files located under `pages/*` are mapped to routes and these files can have special names to make Next.js aware that they are dynamic based on query parameters or the URL itself.
...
### Index page
My index page is located under `pages/index.tsx` which automatically is mapped to the route `/`.
Each page component has to be a default export in one of the files located under `pages/*`.

In order to enable such a page to be statically generated, there must be a named export of an `async` method called `getStaticProps`.
During build-time this method is invoked locally in a Node.js environment to supply the page with props.

This is the first time Next.js introduces us to mixing code that is only available and used during build-time and code that is only available during runtime - therefore the boundaries between a browser environment and a Node environment vanish to increase the developer experience.
...
Finally the method itself allows us to define which props are used to pre-render the index page.
...
Now that we have an index page listing all available posts, we need another page component used to render single posts.
For my blog I decided to route them directly as `/:slug` without any prefix in the URL to improve search engine optimization.
This route is achieved by having the file located under `pages/[slug].tsx`, a unique name that Next.js needs to deduce it has a dynamic parameter called `slug`.

Pages that have multiple variants based on the URL or query parameters require a second special `async` method to be available called `getStaticPaths`.
This method is invoked during build-time and it has to return all path that are available during runtime.
...
A shortened version of my post page located under `pages/[slug].tsx` is the following:
...
All other paths that are requested during runtime will yield a 404 error.

Another unique feature is a hybrid mode using the special `fallback` option returned by `getStaticPaths`.
It is possible to pre-render some paths during build-time and some during runtime on the server when a path is requested for the very first time.
This feature is especially useful if you end up having many thousands of pages and pre-rendering them during build-time is no longer a feasible option which can happen in e-commerce for instance.

Finally, `getStaticProps` has access during build-time to the respective `params.slug` variable that takes on the value of each returned path by `getStaticPaths`.
Using this you can easily fetch the necessary props for pre-rendering a specific variant of the page.
...
Assume you have the following lambda located as `pages/api/name.ts`:
